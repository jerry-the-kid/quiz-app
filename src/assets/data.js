const category = [
  {
    value: 'any',
    title: 'Any Category',
  },
  {
    value: '9',
    title: 'General Knowledge',
  },
  {
    value: '10',
    title: 'Entertainment: Books',
  },
  {
    value: '11',
    title: 'Entertainment: Film',
  },
  {
    value: '12',
    title: 'Entertainment: Music',
  },
  {
    value: '13',
    title: 'Entertainment: Musicals & Theatres',
  },
  {
    value: '14',
    title: 'Entertainment: Television',
  },
  { value: '15', title: 'Entertainment: Video Games' },
  { value: '16', title: 'Entertainment : Board Games' },
  { value: '17', title: ' Science & Nature' },
  { value: '18', title: 'Science : Computers' },
  { value: '19', title: 'Science : Mathematics' },
  { value: '20', title: 'Mythology' },
  { value: '21', title: 'Sports' },
  { value: '22', title: 'Geography' },
  { value: '23', title: 'History' },
  { value: '24', title: 'Politics' },
  { value: '25', title: 'Art' },
  { value: '26', title: 'Celebrities' },
  { value: '27', title: 'Animals' },
  { value: '28', title: 'Vehicles' },
  { value: '29', title: 'Entertainment: Comics' },
  { value: '30', title: 'Science: Gadgets' },
  { value: '31', title: 'Entertainment: Japanese Anime & Manga' },
  { value: '32', title: 'Entertainment: Cartoon & Animation' },
];

const difficulty = [
  { value: 'any', title: 'Any Difficulty' },
  { value: 'easy', title: 'Easy' },
  { value: 'medium', title: 'Medium' },
  { value: 'hard', title: 'Hard' },
];

const type = [
  { value: 'any', title: 'Any Type' },
  { value: 'multiple', title: 'Multiple Choice' },
  { value: 'boolean', title: 'True / False' },
];

export { category, difficulty, type };
